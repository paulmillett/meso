...
PartDisp::PartDisp(const CommonParams& pin) : p(pin)
{
   ...
   GetPot input_params("input.dat");
   N = input_params("PFApp/N",1);              // # of particles
   pmob = input_params("PFApp/pmob",1.0);      // particle mobility coefficient
   rcut = input_params("PFApp/rcut",4.0);      // cut-off radius for particle-particle interactions
   A = input_params("PFApp/A",1.0);            // magnitude of capillary adhesion
   scl = input_params("PFApp/scl",1.0);        // scale capillary forces when applied to fluid
   Khertz = input_params("PFApp/Khertz",1.0);  // Hertz contact force coefficient
   rcut2 = rcut*rcut;
...
void PartDisp::setupParticleCells()
{
...
int PartDisp::cellIndex(int i, int j, int k)
{
   if (i < 0) i += ncellx;
...
void PartDisp::zeroParticleForces()
{
   for (int i=0; i<N; i++) {
...
void PartDisp::calcIJInteraction(int i, int j)
{
...
void PartDisp::applyBoundaryConditions()
{
   for (int i=0; i<N; i++) {
...
Vfield PartDisp::calcParticleInterfaceForce(const Sfield& c, const Vfield& gradc)
{
...
void PartDisp::calcCapillaryForce(const Sfield& cp, const Sfield& c1, const Sfield& c2)
{
   // width of region surrounding particles to probe:
   int wdth = 7;
   // loop over particles:
      ...
      for (int i=0; i<wdth; i++) {
         ...
         for (int j=0; j<wdth; j++) {
            ...
            for (int k=0; k<wdth; k++) {
               ...
               f[pp*3+0] += A*fint*(rx);///rr);
               f[pp*3+1] += A*fint*(ry);///rr);
               f[pp*3+2] += A*fint*(rz);///rr);
            }
         }
      }
   }
...
void PartDisp::writeVTKFile(string tagname, int tagnum)
{
